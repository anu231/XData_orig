...
public class ProcessResultSetNode {
	...
	public static void processResultSetNodeJSQL(Select stmt,boolean debug,parsing.QueryParser qParser) throws Exception{
		...
		//qParser.topLevelRelation = generateRelationHierarchy(plainSelect);//FIXME ANURAG
		...
		*/
		...
		if (whereClause != null) {
			...
			whereClausePred = WhereClauseVectorJSQL.getWhereClauseVector(whereClause,null, qParser.queryAliases,true,0,qParser);
		...
		WhereClauseVectorJSQL.getAggregationDataStructures(plainSelect, qParser.queryAliases, qParser.getQuery().getFromTables(),false,false, qParser);
		...
		for (int k=0;k<rcList.size();k++) {
			...
			if (rcList.get(k) instanceof AllColumns) {
				...
				tempProjectedCols.add(WhereClauseVectorJSQL.getWhereClauseVector(exp,rc.getAlias(), qParser.queryAliases,false,0,qParser));


				/*if(qParser.isUpdateNode){//FIXME - Anurag ignored it				
