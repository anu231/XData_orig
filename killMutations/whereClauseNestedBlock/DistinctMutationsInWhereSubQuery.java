...
public class DistinctMutationsInWhereSubQuery {
	...
	public static void generateDataForkillingDistinctMutationsInWhereSubquery(	GenerateCVC1 cvc) throws Exception{
		...
		for(Conjunct con: cvc.getOuterBlock().getConjuncts()){
			...
			for(Node subQCond: con.getAllSubQueryConds()){
				...
				System.out.println("\n----------------------------------");
				System.out.println("GENERATE DATA FOR KILLING DISTINCT CLAUSE MUTATIONS IN WHERE CLAUSE NESTED SUBQUERY BLOCK: "+ qbt);
				System.out.println("----------------------------------\n");
