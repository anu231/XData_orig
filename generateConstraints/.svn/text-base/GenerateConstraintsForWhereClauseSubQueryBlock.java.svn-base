...
public class GenerateConstraintsForWhereClauseSubQueryBlock {
	...
	/**
	 * In case we need to ensure different groups in subQ and outer tuples such as in cases where the table within the subquery 
	 * and outside are the same and the subQ is >all or >max 
	...
	public static String negateCondForDiffGroupSubQ(GenerateCVC1 cvc, QueryBlockDetails queryBlock, Node subQ) throws Exception{
		...
		if(subQ.getType().equals(Node.getBroNodeSubQType())){
			offset = cvc.getRepeatedRelNextTuplePos().get(subQ.getLeft().getTableNameNo())[1];
			count = cvc.getNoOfTuples().get(subQ.getLeft().getTableNameNo());
		}
		else{
		...
		if(n.getType().equalsIgnoreCase(Node.getBroNodeType())){			
			...
			if(n.getLeft().getType().equalsIgnoreCase(Node.getAggrNodeType())){
				...
				if(af.getFunc().equalsIgnoreCase(AggregateFunction.getAggMAX())){
					...
					String maxStr="MAX_"+columnName+ ": "+af.getAggExp().getColumn().getColumnName()+";\nASSERT(";
					...
					String minStr="MIN_"+columnName+ ": "+af.getAggExp().getColumn().getColumnName()+";\nASSERT(";
					for(int i=1;i<=myCount;i++){
				...
				if(n.getRight().getAgg().getFunc().equalsIgnoreCase(AggregateFunction.getAggMAX())){
					...
					returnStr+="ASSERT ("+ "O_"+ GenerateCVCConstraintForNode.cvcMap(n.getLeft().getColumn(), outerTupleNo+"")+n.getOperator()+"MAX_"+columnName+");\n";
