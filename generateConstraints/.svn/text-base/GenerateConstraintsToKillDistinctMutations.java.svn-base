...
public class GenerateConstraintsToKillDistinctMutations {
	...
	public static String getDistinctConstraints(GenerateCVC1 cvc, QueryBlockDetails queryBlock) {
		...
		for(int i=0; i<projectedCols.size(); i++){
			...
			if(n.getType().equalsIgnoreCase(Node.getColRefType())){
				...
				else{/** If there is a single tuple in this relation, this means that this node is involved in equi joins with other relation nodes*/
	 ...
	 *  If more than one column is present in this equivalence class, it returns the column with atleast two tuples and which is not unique	  
	 * @param cvc
	 * @param queryBlock
	 * @param col
	 * @return
